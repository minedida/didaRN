...
class MyRow extends Component {
  ...
  onReleaseRow = async () => {
    ...
    if (this.onLongPress) {
      setTimeout(async () => {
        ...
        if (this.props.rowHasMoved === false) {
  ...
  onSortPress = async () => {
    ...
    if (this.props.openRowKey === this.props.rowId) {
      await this.closeRow();
      if (this.props.toggleRowActive) {
  ...
  render() {
    return (
      <SwipeRow
        ...
        onRowOpen={() => this.props.onRowOpen(this.props.rowId)} //not allowed
        onRowClose={() => this.props.onRowClose(this.props.rowId)} //not allowed
